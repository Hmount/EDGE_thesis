...
#chy
chytrts <- trtsite %>% filter(EDGE_site=="CHY")
sum(!is.na(chytrts))
#for site CHY how many populations have onsite (DB_HPG)
chytrts <- trtsite %>% filter(EDGE_site=="CHY") %>% 
  summarise(fromsite = (sum(!is.na(DB_HPG))/26)*100) #26 is total pops in chy
#AS_MO and DB_CPER regional
chytrts <- trtsite %>% filter(EDGE_site=="CHY") %>% 
  summarise(fromsite = (sum(!is.na(AS_MO))/26)*100) 
chytrts <- trtsite %>% filter(EDGE_site=="CHY") %>% 
  summarise(fromsite = (sum(!is.na(DB_CPER))/26)*100) 
#wider

#for site SGS how many populations have DB/CPER
#for site HYS how many populations have ???
#for site KNZ how many populations have ???
#for site SBL how many populations have sevLTER/blue
  # RG/sev also good
#for site SBK how many populations have sevLTER/black
  # RG/sev also good

sites <- unique(trtsite)
test <- trtsite %>% group_by(site) %>% summarise()
